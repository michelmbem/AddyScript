...
namespace AddyScript.Runtime.Utilities
{
    public static class Reflector
    {
        ...
        public static void SetValue(Type type, string memberName, object target, Dynamic value)
        {
            if (type.IsCOMObject)
            {
                const BindingFlags flags = BindingFlags.Public | BindingFlags.Static |
                ...
                type.InvokeMember(memberName, flags, null, target, new[] { value.AsNativeObject });
            }
            else
            {
                MemberInfo member = GetValueMember(type, memberName);
