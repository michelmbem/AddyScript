...
using Plotter.Curves;
using Plotter.Properties;
...
namespace Plotter
{
    public partial class PlotterForm : Form
    {
        ...
        private bool ValidateInput()
        {
            ...
            if (string.IsNullOrEmpty(cboFunction1.Text.Trim()))
            {
            ...
            if (cboFunction2.Enabled && string.IsNullOrEmpty(cboFunction2.Text.Trim()))
            {
        ...
        private void InitPlotting()
        {
            ...
            window = new Window((double) numLeft.Value, (double) numRight.Value,
                                (double) numBottom.Value, (double) numTop.Value);
        ...
        {
            int tmpX, tmpY;
            projector.Project(x, y, out tmpX, out tmpY);
            if (curX != int.MinValue && curY != int.MinValue &&
                tmpX != int.MinValue && tmpY != int.MinValue)
        ...
        {
            int boxX, boxY;
            projector.Project(x, window.Bottom, out boxX, out boxY);
        ...
        {
            int boxX, boxY;
            projector.Project(window.Left, y, out boxX, out boxY);
        ...
        private void display_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (chkAxis.Checked)
                DrawAxis(e.Graphics, (double)numXInterval.Value, (double)numYInterval.Value);
            Plot(e.Graphics);
        }
