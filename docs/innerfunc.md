...
At the time of writing this manual page, AddyScript has a set of 45 predefined functions.
We have already discovered some of them in the previous sections.
Here is a more general overview of AddyScript's built-in functions:
...
* `int randint(int min, int max = 0)` : generates a random **int** between _min_ (inclusive) and _max_ (exclusive). If _max_ is omitted, the random value will be generated between `0` and _min_. In all cases, the limits are always sorted so that _max_ is greater than or equal to _min_.
...
* `float|decimal round(float|decimal value, int precision = 0)` : gets _value_ rounded to _precision_ decimal digits. The result is of the same type as the first argument.
